...
# LP-067: Confidential ERC-20 with FHE Balances
...
Third parties see only ciphertexts. The threshold decryption (LP-066) delivers the plaintext only to the requesting address via an encrypted callback channel.

### Compliance Integration

For regulated tokens (LP-001), the compliance registry can be granted a decryption permission by the token holder, enabling KYC-gated balance visibility without making balances public.
...
3. Metadata leakage: sender, receiver, and timing are still visible. Use LP-068 for full privacy.
