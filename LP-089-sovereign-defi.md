...
Defines the sovereign governance model for Lux subnet chains. Each subnet chain (Zoo, Hanzo, SPC, Pars) maintains its own independent governance: its own governor contract, its own treasury, its own parameter set. The primary C-Chain DAO (LP-085) governs cross-chain protocol parameters only. Subnet chains are sovereign within their domain.
...
Each subnet chain deploys its own Governor contract (LP-086 template) with chain-specific voting token:
...
- Cross-chain fee collection ratios (LP-080).
