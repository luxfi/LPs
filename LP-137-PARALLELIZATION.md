...
| Q-Chain | QuantumVM (Pulsar) | (in cevm v0.43+) | `crypto/corona/gpu/metal/corona_verify.metal` + `corona_sign.metal` + `mpcvm/src/mpcvm_corona.metal` | Pulsar polynomial ops parallel (NTT/multiply); top-level verify uses single-thread challenge sampling (CUDA: `if (tid != 0) return;` for canonical challenge) | partial template; lower NTT is parallel; challenge sampling is canonical-by-design (Fiat-Shamir transcript byte-equal) |
...
| 22 | corona | NOTIMPL umbrella; cevm path lives in `cevm/lib/consensus/quasar/gpu/` | Metal `crypto/corona/gpu/metal/{corona.metal, corona_ops.metal, corona_sign.metal, corona_verify.metal}`; CUDA + WGSL ports; cevm `verify_corona_batch` GPU-batched at the NTT/polynomial layer | (cevm-side; intra-proof Fiat-Shamir is canonical-sequential per protocol) | **C** intra-proof; cross-proof batch is template-fit |
...
| Round-by-round protocol (one-thread canonical) | 5 | `mpcvm_frost`, `mpcvm_cggmp21`, `mpcvm_corona` (top-level), `cuda/kernels/crypto/corona_sign` (challenge sampling), `quasar_wave` per-service drain | structural-sequential — protocol round N depends on round N-1's transcript |
...
| Q-Chain | Pulsar polynomial arenas in `mpcvm_gpu_engine.mm` + `cevm/lib/consensus/quasar/gpu/quasar_corona_verifier` (host blst for cert ingress; on-device verify path via `crypto/corona/gpu/`) | `quasar_cpu_reference.cpp` Pulsar verify | yes — `quasar-determinism-test` Pulsar vectors |
