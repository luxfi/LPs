...
The router is deployed on 270 chains at launch. Chain registry includes all EVM-compatible chains, Lux subnets, and chains reachable via the Teleport protocol (LPS-017 native programs). The full chain ID list is maintained in the deployment manifest and is not hardcoded in the contract.

## Security Considerations

1. **MPC group opacity**: The contract cannot distinguish between a 3-of-5 and a 5-of-5 threshold. This is by design -- threshold policy is enforced off-chain by the MPC protocol (LPS-019). On-chain verification only confirms the aggregate signature is valid.
...
| LPS-003 Securities Bridge | `LPS-003-securities-bridge.md` |
