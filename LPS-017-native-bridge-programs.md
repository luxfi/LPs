...
# LPS-017: Native Bridge Programs for Teleport

## Abstract

Defines 18 native on-chain programs that implement the Teleport bridge protocol on non-EVM chains. Each program is written in the chain's native language and uses the chain's native signature verification to validate MPC threshold signatures. FROST (EdDSA/Schnorr) is used for Ed25519-native chains. CGGMP21 (ECDSA) is used for secp256k1-native chains. All programs implement the same lock/mint/burn/release pattern defined in LPS-003 and LPS-016.

## Motivation

Teleport bridges EVM chains via the OmnichainRouter (LPS-016). Non-EVM chains require native programs because:
...
3. Request a threshold signature from the MPC group (LPS-019)
