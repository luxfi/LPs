...
The Teleport VM runs the T-chain, the universal cross-chain transport layer for Lux. While Warp (LPS-021) handles intra-Lux messaging and Bridge VM (LPS-034) handles asset custody, Teleport extends cross-chain communication to external networks (Ethereum, Solana, Bitcoin, Cosmos). The T-chain maintains light client headers for external chains, verifies inclusion proofs, and relays messages through MPC-signed transactions.
