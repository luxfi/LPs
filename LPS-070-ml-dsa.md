...
# LPS-070: FIPS 204 ML-DSA Signatures
...
|-------------|-------------|
| Validator signing | Post-quantum block/vote signatures (Phase 2, LPS-012) |
| DID authentication | ML-DSA keys in DID documents (LPS-060) |
| Bridge attestation | PQ-safe bridge message signatures |
| Certificate signing | Long-lived credentials requiring PQ safety |

### Key Storage

ML-DSA keys are stored in KMS (LPS-062) with HSM backing. The larger key sizes (1.3-2.6 KB public, vs 32 B for Ed25519) require adjusted storage quotas.
