...
# LPS-084: Validator Reward Game Theory
...
| Parameter | Value |
|-----------|-------|
| base_reward | Per-epoch issuance (LPS-083) / number of active validators |
...
Slashed stake is burned via DeadBurn (LPS-079). Slashing is adjudicated on-chain via the `SlashingManager` contract with a 48-hour dispute window.
