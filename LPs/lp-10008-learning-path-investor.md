---
lp: 10008
title: Learning Path - Investor & Institutional
description: Understanding value accrual and capital deployment in Lux Network
author: Lux Core Team
status: Research
type: Meta
created: 2025-12-21
tags: [learning-path, investor, institutional, esg]
order: 28
tier: core
---

# LP-0018: Learning Path - Investor & Institutional

## Abstract

**"Where does value accrue and why?"**

This path provides capital deployment understanding without deep technical requirements.

## Motivation

Informed investment requires understanding tokenomics, and ecosystem dynamics. This path provides the analytical framework for evaluating opportunities.

## Audience

Funds, allocators, board members, institutional partners.

## Prerequisites

None. This path includes Core Protocol essentials.

## Outcome

After completing this path, you will understand:
- Token economics and value flows
- Platform revenue model
- DeFi surface area
- Trading liquidity
- Governance structure
- ESG commitments

---

## Curriculum

### Stage 1: Economics (Required)

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 1 | [LP-0001](/docs/lp-0001-primary-chain-native-tokens-and-tokenomics/) | Tokenomics | 25 min | Deep |
| 2 | [LP-0000](/docs/lp-0000-network-architecture-and-community-framework/) | Network Architecture | 15 min | Skim |

### Stage 2: Platform Value

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 3 | [LP-1000](/docs/lp-1000-p-chain-core-platform-specification/) | Platform Economics | 20 min | Medium |
| 4 | [LP-1100](/docs/lp-1100/) | Staking & Rewards | 20 min | Deep |

### Stage 3: DeFi Surface Area

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 5 | [LP-3600](/docs/lp-3000/) | DeFi Overview | 15 min | Skim |
| 6 | [LP-3337](/docs/lp-3337/) | AMM Economics | 15 min | Skim |

### Stage 4: Trading Liquidity

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 7 | [LP-9000](/docs/lp-9000-dex-overview/) | DEX Overview | 15 min | Skim |
| 8 | [LP-9600](/docs/lp-9600/) | Market Making Incentives | 15 min | Medium |

### Stage 5: Governance & Institutional

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 9 | [LP-2860](/docs/lp-2860-fund-management-index/) | Fund Management | 20 min | Deep |
| 10 | [LP-2850](/docs/lp-2850-dao-governance-index/) | DAO Governance | 20 min | Deep |

### Stage 6: ESG & Impact

| Order | LP | Title | Time | Depth |
|-------|-----|-------|------|-------|
| 11 | [LP-2995](/docs/lp-2995-esg-impact-index/) | ESG Framework | 20 min | Deep |
| 12 | [LP-72100](/docs/lp-72100/) | Carbon Accounting | 15 min | Medium |
| 13 | [LP-2920](/docs/lp-2920/) | Green Compute | 15 min | Medium |

---

## Total Time

**~4 hours** for complete coverage.

---

## Investment Thesis Summary

| Layer | Value Driver | Key LPs |
|-------|--------------|---------|
| Platform | Staking rewards, fee capture | LP-1xxx |
| DeFi | TVL, protocol fees | LP-3xxx |
| Trading | Volume, spreads | LP-9xxx |
| Governance | Treasury, grants | LP-71xxx |
| ESG | Impact metrics, compliance | LP-72xxx |

---

## Security Considerations

None — this is a meta/organizational proposal.

## Copyright

Copyright and related rights waived via [CC0](https://creativecommons.org/publicdomain/zero/1.0/).
