...
| T-Chain MPC Custody (LP-5013) | https://github.com/luxfi/lps/LPs/lp-5013-t-chain-decentralised-mpc-custody-and-swap-signature-layer.md |
| Warp Messaging (LP-6022) | https://github.com/luxfi/lps/LPs/lp-6022-warp-messaging-20-native-interchain-transfers.md |
...
## International Regulatory Compatibility

### United States
- SEC-registered Alternative Trading System (Form ATS-N)
- FINRA member Broker-Dealer
- Section 17A registered Transfer Agent
- Reg D (506b, 506c), Reg S, Reg A+, Reg CF offerings supported
- Rule 144 transfer restrictions enforced on-chain via LockupModule

### United Kingdom
- FCA authorization pathway for Security Token Offerings
- MiFID II compliance via JurisdictionModule (GB country code)
- Compatible with UK's Financial Markets Infrastructure Sandbox

### European Union / Luxembourg
- Digital securities exchange registration in Luxembourg
- MiCA (Markets in Crypto-Assets) compliant token classification
- CSSF-supervised digital asset platform
- EU Prospectus Regulation compatibility
- ESMA guidelines on crypto-assets compliance
- Cross-border passporting via JurisdictionModule EU member state codes

### Isle of Man
- VASP (Virtual Asset Service Provider) registration
- Isle of Man Financial Services Authority (IOMFSA) oversight
- Digital currency exchange license

### Cross-Jurisdiction Transfer
The JurisdictionModule enforces per-address ISO 3166-1 alpha-2 country codes.
Transfers between jurisdictions are validated against configurable allowlists:
- US ↔ EU transfers require dual compliance (SEC + MiCA)
- UK post-Brexit requires separate FCA compliance path
- IOM VASP can serve as gateway between jurisdictions

### tZero Cross-Listing (Confirmed Partnership)
Liquidity.io is one of 6 SEC-registered digital ATSs.
tZero partnership enables native cross-listing:
- Webhook-based compliance data sharing (REST + HMAC-SHA256)
- Shared KYC/AML status via aml.flagged/aml.cleared/kyc.approved events
- Cross-ATS settlement bridging via SecurityBridge contract
- T+0 settlement on both platforms
