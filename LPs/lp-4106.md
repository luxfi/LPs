...
**Key innovation**: This protocol **solves the EthResearch 2022 split-view/equivocation attack** by making the signable digest a single canonical hash derived from finalized on-chain state, and using the Lux chain as the bulletin board so signers cannot be induced to open different parts of the Lamport material for different messages in the same session.
