...
Cross-refs: LP-012 (PQ GPU), LP-019 (Threshold MPC), LP-020 (Quasar consensus), LP-070 (ML-DSA), LP-073 (Ringtail), LP-075 (BLS), LP-149 (hash precompiles), LP-178 (per-algo CI).
...
|----------|-----------|----------------|--------|-----|
| **Consensus aggregate** | BLS12-381 (48-byte aggregate) | Ringtail (~33 KB threshold) parallel-signed alongside BLS | LIVE — Quasar triple sign | LP-020, LP-073, LP-075 |
...
| **Bridge MPC (ECDSA threshold)** | CGGMP21 over secp256k1 | Ringtail (linear shares, no Paillier MtA) for outgoing-bridge sigs | PARTIAL — Ringtail Go canonical + C++ M1-M3 byte-equal; M4 Sign+Verify in flight | LP-019, LP-073 |
| **Bridge MPC (Schnorr threshold)** | FROST over Ed25519 | FROST-PQ via threshold ML-DSA OR Ringtail multi-sig | DESIGN | LP-076 |
| **DKG / key resharing** | Pedersen DKG over secp/Ed | Pedersen DKG over R_q (Ringtail's ring) | DESIGN — sketched in LP-019 §5 | LP-019 |
...
1. **Triple-sign consensus.** Every Q-chain block carries (BLS aggregate ∥ Ringtail aggregate ∥ ML-DSA-65 per-validator). Acceptance is `BLS.Verify ∧ (Ringtail.Verify ∨ MLDSA.Verify)`. Quantum adversary breaking BLS in private gets one half; needs to break Ringtail or ML-DSA to forge a block. Falls back to ML-DSA when Ringtail liveness fails.
2. **Hybrid TLS.** ML-KEM-768 piggybacks on X25519 — both must be broken to recover the session key. Already shipping in Go 1.26.
3. **Layered bridge.** For now, bridge MPC stays on CGGMP21 ECDSA (record-now-decrypt-later threat is bounded by the 7-day lockup). When Ringtail M4+M5 land, bridge can run in parallel-sig mode (BLS+Ringtail) with an opt-in PQ-only mode for high-value flows.
...
2. **Hardware wallet support.** Ledger/Trezor have no ML-DSA firmware. Ringtail has no firmware anywhere yet.
3. **Aggregate proof size.** Ringtail aggregate is ~33 KB vs BLS 48 bytes. Quasar's bandwidth budget tolerates it (per-block PQ is ~5% of typical EVM block size); but light clients optimizing for bytes-on-chain will keep BLS as the canonical primary aggregate for years.
4. **Performance.** Ringtail signing is ~600 ms aggregate at K=21 validators on M2 Ultra (Go canonical). LP-137 GPU plan brings this down ~10×. Until M5 (Metal/CUDA/WGSL byte-equal) lands, Ringtail is CPU-only and the bandwidth-throughput tradeoff disfavors making it the primary lane.
...
Recorded decision: **BLAKE2b in Lattigo PRNG, BLAKE3 in Ringtail transcripts, both in LP-149 EVM precompiles.** Validated empirically by `ringtail_blake_bench` (luxcpp/crypto):
...
- "Ringtail is faster than BLS." It isn't, won't be, and doesn't need to be — it's the PQ liveness lane, not the throughput lane.
...
The accurate one-line summary: **consensus and TLS are PQ-resilient with hybrid layering; bridge is PQ-ready in Q3 2026 once Ringtail M4-M5 land; wallet is the long-tail problem, gated on hardware-wallet vendors and an EVM tx envelope EIP.**
